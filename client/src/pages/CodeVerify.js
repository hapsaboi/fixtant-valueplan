...
import illustration from "images/fixtantletter.jpeg";
import logo from "../images/Full Black.svg";
import { ReactComponent as SignUpIcon } from "feather-icons/dist/icons/user-plus.svg";
import Axios from 'axios';
import { authenticate,code as codeauth } from '../data/api';
...
const IllustrationImage = styled.div` 
  ...
  ${tw`m-12 xl:m-16 w-full max-w-lg bg-cover bg-center bg-no-repeat`}
`;
...
function SignUp() {
  ...
  async function registerFunc(e) {
    ...
    try {
      ...
      const registerData = { name, email: nemail, phone, dob, password,code };
  ...
  async function checkCodeFunc(e) {
    setMessage("");
    e.preventDefault();

    try {
      const response = await Axios.get(codeauth.checkCode+"/"+code);
      setMessage(response.data.msg + '.');
      setTimeout(() => {
        setMessage("");
      }, 5000);
  ...
  let data = {
    ...
    headingText: "Sign Up - Value Plan",
              ...
              {!verify ?
                <>
                  <p tw="text-sm text-gray-600 text-center">
                    Have a code for our value plan, insert it here to get started with fixtant's value plan.
                  </p>
                  <Actions>
                    <input type="text" placeholder="Your Code E.g 123ABC" id="code" onChange={(e)=>setCode(e.target.value)} />
                    <button onClick={checkCodeFunc}>Get Started</button>
                  </Actions>
                </>
                :
                <>
                  ...
                  {verify? 
                    <div style={{ textAlign: 'center' }}>{"Registering with code: "+code+" - type: "+codedata.type}</div>
                  : null
                  }
                  <FormContainer>
                    <Form>
